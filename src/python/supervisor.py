...
class OfferingSupervisor:
    def __init__(self, model_xml=None, tokenizer_id=None, shm_name=DEFAULT_SHM_NAME, device="NPU"):
        ...
        self.input_type_map = {} # Map input names to numpy dtypes
    ...
    def map_ov_type_to_numpy(self, ov_type):
        type_str = str(ov_type)
        if "i64" in type_str: return np.int64
        if "i32" in type_str: return np.int32
        if "f32" in type_str: return np.float32
        if "f16" in type_str: return np.float16
        if "boolean" in type_str: return bool
        return np.int64 # Default safe fallback for LLMs
    ...
    def load_inference_engine(self):
        ...
        if self.device == "NPU":
            ...
            try:
                ...
                # Verify Inputs & Build Type Map
                for input_node in self.model.inputs:
                    dtype = self.map_ov_type_to_numpy(input_node.element_type)
                    self.input_type_map[input_node.any_name] = dtype
                    print(f"  > Input: {input_node.any_name} {input_node.partial_shape} -> {dtype}")
    ...
    def warmup_model(self):
        """Sends a dummy inference request to initialize NPU command queues."""
        print("[Supervisor] Warming up NPU with dummy data...")
        try:
            dummy_inputs = {}
            for name, dtype in self.input_type_map.items():
                # Assumes [1, 128] static shape based on configuration
                dummy_inputs[name] = np.zeros((1, STATIC_SEQ_LEN), dtype=dtype)

            # Handle beam_idx if implicitly required
            if "beam_idx" not in dummy_inputs:
                 # Check if driver expects it by blindly adding it safely or just try basic
                 pass
    ...
    def run_custom_static_inference(self, formatted_prompt):
        ...
        for i in range(max_new_tokens):
            ...
            # B. Prepare Inputs with STRICT TYPE CASTING
            # Default to Int64 if map not populated (e.g. Optimum path)
            id_type = self.input_type_map.get("input_ids", np.int64)
            mask_type = self.input_type_map.get("attention_mask", np.int64)
            pos_type = self.input_type_map.get("position_ids", np.int64)

            inputs_dict = {
                "input_ids": static_input_ids.numpy().astype(id_type),
                "attention_mask": static_attention_mask.numpy().astype(mask_type),
                "position_ids": static_position_ids.numpy().astype(pos_type)
            }
